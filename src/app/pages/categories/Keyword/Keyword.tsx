...
import { keywordApi } from 'src/app/apis'
...
const KeywordPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true)
                var res = await keywordApi.getAll()
                setDataTable(res ?? [])
    ...
    const handleDelete = async (id: string) => {
        var res = await keywordApi.delete(id)
        if (res) {
    ...
    return (
        <div>
            <PageTitle breadcrumbs={[]}>Danh sách từ khóa</PageTitle>
