...
import { Sentence } from 'src/app/models'
import { sentenceApi, intentApi } from 'src/app/apis'
...
const FormModal: React.FC<Props> = ({
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setIsLoading(true)
                var res = await sentenceApi.getById(modalId)
                if (res) {
    ...
    const postData = async (data: Sentence) => {
        try {
            setButtonLoading(true)
            var res = await sentenceApi.add(data)
            if (res) {
    ...
    const putData = async (data: Sentence) => {
        try {
            setButtonLoading(true)
            var res = await sentenceApi.update(modalId, data)
            if (res) {
                notification.success({
    ...
    return (
        ...
        >
            <Form {...layout} form={form}>
                ...
                <Form.Item label='Ý định' name='intent_id'>
                    <Select>
                        {intents.map((item: any) => (
                            <Option key={item.id} value={item.id}>{item.intent}</Option>
                        ))}
                    </Select>
                </Form.Item>
            </Form>
